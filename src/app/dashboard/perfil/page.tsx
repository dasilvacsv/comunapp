...
import { User, Lock, Save, CheckCircle, AlertCircle, Key, Shield } from 'lucide-react';
...
export default function PerfilPage() {
  ...
  const onSubmit = async (data: PasswordFormData) => {
    ...
    try {
      ...
      reset(); 
      setTimeout(() => setSuccessMessage(null), 4000); // El mensaje desaparece después de 4 segundos
    } catch (error) {
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
          ...
          <div className="mt-8 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center">
              ...
              <p className="flex-1 mt-1 sm:mt-0 p-3 bg-gray-100 rounded-md text-gray-800 font-medium">{session?.user?.name || 'No disponible'}</p>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center">
              ...
              <div className="flex-1 mt-1 sm:mt-0 p-3 bg-gray-100 rounded-md flex items-center gap-2">
                ...
                <span className="text-gray-800 font-medium">Administrador (Vocero)</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 mt-8">
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="currentPassword" type="password" placeholder="••••••••" disabled={isSubmitting} {...register('currentPassword')} className="pl-10" />
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <Input id="newPassword" type="password" placeholder="Mínimo 6 caracteres" disabled={isSubmitting} {...register('newPassword')} className="pl-10" />
                </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="confirmPassword">Confirmar Contraseña</Label>
                 <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <Input id="confirmPassword" type="password" placeholder="Repite la contraseña" disabled={isSubmitting} {...register('confirmPassword')} className="pl-10" />
                </div>
            ...
            <div className="flex justify-end pt-4">
              <Button type="submit" disabled={isSubmitting} className="min-w-[180px]">
                {isSubmitting ? (
                  <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>Actualizando...</>
                ) : (
